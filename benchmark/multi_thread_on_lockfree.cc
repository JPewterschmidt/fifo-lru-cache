...
t::nanoseconds multi_threads_on_lockfree(size_t thrnum)
{
    ::std::latch l{ thrnum };
    ...
    ::std::cout << "multi_threads_on_lockfree(" << thrnum << ") Complete." << ::std::endl;

    return result;
}

void multi_threads_on_lockfree_in_total(size_t thrnum)
{
    ::std::ofstream ofs{"multi_threads_on_lockfree_in_total.csv"};
    csv2::Writer w(ofs);
    ...
    for (size_t i{}; i < 10; ++i)
    {
        ...
        auto result_pair_rows = thrs
            | rv::transform([](auto&& thrnum)   { return ::std::pair{ thrnum,   multi_threads_on_lockfree(thrnum) }; })
